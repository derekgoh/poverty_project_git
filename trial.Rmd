...
g1 <- 
  g %>%
      group_by_("SPM_household_resources_y2", "gender") %>%
      na.omit() %>%
      summarize(Percentage = n()) %>% 
      group_by_("SPM_household_resources_y2") %>%
      mutate(Percentage = Percentage / sum(Percentage) * 100) %>%
      arrange_("SPM_household_resources_y2") %>%
...
edited_stackbar <- completeFun(d, c("in_poverty_SPM", "in_poverty_SPM_y1", "in_poverty_SPM_y2")) %>%
